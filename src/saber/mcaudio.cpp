...
I2SAudio::I2SAudio(Adafruit_ZeroI2S &_i2s, Adafruit_ZeroDMA &_dma, Adafruit_SPIFlash &_spiFlash) : i2s(_i2s),
                                                                                                   audioDMA(_dma),
                                                                                                   spiFlash(_spiFlash)
{
...
bool I2SAudio::play(int fileIndex, bool loop, int channel)
{
    ...
    Log.p("Play [").p(fileIndex).p("]: channel=").p(channel).p(" looping=").p(loop ? 1 : 0).p(" lenInBytes=").p(header.lenInBytes).p(" nSamples=").p(header.nSamples).p(" format=").p(header.format).eol();
